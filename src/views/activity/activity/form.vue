<template>
  <div>
    <div style="text-align: right;">
      <Button type="primary" icon="md-add" @click="modal1 = true">新增活动</Button>
    </div>
    <Modal
            v-model="modal1"
            title="新增活动"
            @on-ok="ok('activityFormValidate')"
            @on-cancel="cancel('activityFormValidate')">
      <Form ref="activityFormValidate" :model="activityFormValidate"
            :rules="activityRuleValidate" :label-width="80">
        ...
        <FormItem label="内容介绍" prop="content">
          <Input v-model="activityFormValidate.content" type="textarea" :autosize="{minRows: 3}"
...
<script lang="ts">
    ...
    export default class ActivityForm extends Vue {
        ...
        private activityRuleValidate = {
            ...
            imgUrl: [
                {required: true, message: '图片还没上传', trigger: 'blur'}
            ]
        };
